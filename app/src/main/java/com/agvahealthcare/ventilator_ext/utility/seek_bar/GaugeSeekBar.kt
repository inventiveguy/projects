...
class GaugeSeekBar : View {
    ...
    private companion object {
        private const val DEFAULT_START_ANGLE_DEG = 25f
        private const val DEFAULT_THUMB_RADIUS_DP = 5
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        init(measuredWidth / 2f - 5, measuredHeight / 2f - 5)
    }
    ...
    private fun handleMotionEvent(event: MotionEvent) {
        val relativeX = measuredWidth / 2f - (event.x-5)
        val relativeY = (event.y +5) - measuredHeight / 2f
    ...
    private fun init(centerX: Float, centerY: Float) {
        ...
        val radiusPx = min(centerX, centerY) - 10
        val margin = max(thumbRadius, trackWidth / 2f) - 5
